...
@Component({
	...
	imports: [NgForOf, NgClass, HotelCardComponent, NgIf],
...
export class HotelsListByCityComponent implements OnInit {
	...
	nextSlide() {
		if (this.hotels()) this.currentIndex = (this.currentIndex + 1) % this.hotels().length;
	}

	prevSlide() {
		if (this.hotels()) this.currentIndex = (this.currentIndex - 1 + this.hotels().length) % this.hotels().length;
	}
