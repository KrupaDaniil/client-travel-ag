...
export class RoleManagementComponent implements OnInit, AfterViewChecked {
  ...
  private selectTableRow(): void {
    if (this.rolesBlock?.nativeElement && !this.isSelectedRow) {
      ...
      this.render_2.listen(this.rolesBlock.nativeElement, 'click', (event: Event) => {
        const target = event.target as HTMLElement;
        if (target.tagName.toLowerCase() === "td") {
          const row = target.closest('tr') as HTMLTableRowElement;
          if (row) {
            const rd = row.querySelector('input[type="radio"]') as HTMLInputElement;
            if (rd) {
            ...
            if (this.selectRole) {
              if (this.removeRoleBtn?.nativeElement) {
                this.render_2.listen(this.removeRoleBtn.nativeElement, 'click', () => {
                  if (this.selectRole?.id) {
                    this.roleService.deleteUserRole(this.selectRole.id);
                    this.selectRole = undefined;
                    this.isSelectedRow = false;
                    this.render_2.setProperty(this.rolesBlock!.nativeElement, 'checked', false);
                  }
                });
              }
            }
