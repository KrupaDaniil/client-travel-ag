...
export class ClimateManagementComponent implements OnInit, AfterViewChecked {
	...
	climatesList: Signal<IClimateEntity[]> = computed(() => this.store.climatesEntities());
	...
	constructor(private climateService: ClimateService, private message: MessageService, private render: Renderer2) {
	...
	private loadingStatus(): void {
		if (this.climatesList() && this.climatesList().length > 0) {
	...
	private showAddClimateBlock(): void {
		if (this.addClimateBlock?.nativeElement && this.addClimateBtn?.nativeElement) {
	...
	private selectedTableRow(): void {
		if (this.climateBlock?.nativeElement && !this.isSelectedRow) {
			this.render.listen(this.climateBlock.nativeElement, "click", (e: Event) => {
				...
				if (t.tagName.toLowerCase() === "td") {
					...
					if (r) {
						...
						const selectedId = Number.parseInt(r.dataset["climateId"] as string);
						...
						if (this.selectedClimate) {
							if (this.removeClimateBtn?.nativeElement) {
								this.render.listen(this.removeClimateBtn.nativeElement, "click", () => {
									if (this.selectedClimate?.id) {
										this.climateService.deleteClimate(this.selectedClimate.id);
										this.selectedClimate = undefined;
										this.isSelectedRow = false;
										this.render.setProperty(this.climateBlock!.nativeElement, "checked", false);
									}
								});
							}
						}
