...
export class CityManagementComponent implements OnInit, AfterViewChecked {
  ...
  constructor(private cityService: CityService, private message: MessageService, private countryService: CountryService,
  ...
  private createCityForm(): void {
    this.addCityForm = new FormGroup({
      ...
      country: new FormControl<IMainCountryForCityEntity | null>(
        null,
        Validators.required
      ),
            ...
            if (r) {
              ...
              if (this.selectedCity) {
                ...
                if (this.removeCityBtn?.nativeElement) {
                  this.render.listen(
                    this.removeCityBtn.nativeElement, 'click', () => {
  ...
  setImage(event: Event): void {
    const currentCity: ICityEntity<IMainCountryForCityEntity, IBlobImageEntity> | undefined =
      this.getCurrentCity(event, "imgCityId");
  ...
  setDescription(event: Event): void {
    const currentCity: ICityEntity<IMainCountryForCityEntity, IBlobImageEntity> | undefined =
      this.getCurrentCity(event, "dsCityId");
  ...
  private getCurrentCity(event: Event, dtsName: string): ICityEntity<IMainCountryForCityEntity, IBlobImageEntity>
    | undefined {
