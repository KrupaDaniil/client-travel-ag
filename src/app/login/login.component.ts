...
@Component({
  selector: 'app-login',
  imports: [FormsModule, ReactiveFormsModule, MatCard, MatCardHeader, MatCardContent, MatCardTitle,
    MatCardActions, MatButton, NgOptimizedImage, MatFabButton],
...
export class LoginComponent implements OnInit{
  ...
  ngOnInit() {
    ...
    this.loginForm = new FormGroup({
      login: new FormControl("", {validators: [Validators.required, Validators.pattern(this.isEmpty)]}),
      password: new FormControl("", {validators: [Validators.required, Validators.pattern(this.isEmpty)]}),
    });
  ...
  onSubmit():void {
    if (this.loginForm && this.loginForm.valid) {
      ...
      this.userService.singIn(loginData).subscribe({next:(res: boolean): void => {
          if (!res) {
            this.showErrorMessage();
          }
        }});
