...
export class UserService {
      ...
      if (item !== null) {
        if (this.isUserStartData(item)) {
          ...
          return this.http.checkUsername(user.username).pipe(
            ...
            res ? this.messageService.setMessage("Incorrect password entered") :
  ...
  singUp(user: IUserReg):void {
    this.http.registrationUser(user).subscribe({next:(item: boolean|IError):void => {
  ...
  private isUserStartData(item: any): boolean {
